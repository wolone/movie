...
- Cloudflare Cron 分批推进三个资源站的全量同步任务：西瓜每 15 分钟处理 15 页，无水印每 5 分钟处理 15 页，UUZY 每 5 分钟处理 1 页，支持 D1 游标断点续传
...
三个资源站不提供稳定的豆瓣 ID，ApiZero 当前公开接口也按豆瓣 ID 或豆瓣 URL查询，因此资源采集与豆瓣关联是两个步骤，不会在生产环境做未经确认的标题猜测。西瓜资源和无水印资源网会依据接口返回的 `pagecount` 分页同步，每个 Cron 批次最多处理 15 页；西瓜列表和详情接口使用 `pagesize=50` 减少全量扫描页数；UUZY 的 API 虽然返回 `pagecount`，但忽略 `pg` 参数，采集器因此改用 UUZY 网站的 `?page=` 分页并抓取详情页播放链接。为控制 Worker 上游请求数，UUZY 每个批次处理 1 页。
