...
三个资源站不提供稳定的豆瓣 ID，ApiZero 当前公开接口也按豆瓣 ID 或豆瓣 URL查询，因此资源采集与豆瓣关联是两个步骤，不会在生产环境做未经确认的标题猜测。西瓜资源和无水印资源网会依据接口返回的 `pagecount` 分页同步，西瓜列表和详情接口使用 `pagesize=50` 减少全量扫描页数；UUZY 的 API 虽然返回 `pagecount`，但忽略 `pg` 参数，采集器因此改用 UUZY 网站的 `?page=` 分页并抓取详情页播放链接。
